            ...
            if($(el).find('.product-name a').length > 0){
                ...
                if(discount == '0'){
                    ...
                    .done(function(data) {
                        ...
                        var href = this.url;
                        var hrefrep  = repl_url( href );
                        console.log($(data).find('.discount span').html())
                        console.log($(data).find('.discount').html());
    ...
    function reload_modal(){
       ...
       .done(function(done) {
        ...
        if($(done).find('[action="https://www.somproduct.ro/checkout/cart/updatePost"]').length > 1){
            ...
            var html = $(done).find('[action="https://www.somproduct.ro/checkout/cart/updatePost"] .cart-items').html();
            console.log(html);
            reload_discont(html);
        }
    });
