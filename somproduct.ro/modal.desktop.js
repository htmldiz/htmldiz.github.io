jQuery(function($) {
	...
	setInterval(function(){
		$('.cart.cart-dropdown-toggle').removeClass('open');
	})
	jQuery('#cart-dropdown-toggle').off('hover');
	jQuery('#cart-dropdown-toggle a').off('hover');
	jQuery('#cart-dropdown-toggle a').hover(function(){
		return false;
	})
	function clearmodalIds(datamessages = ""){
				...
				if($('body').find('#discount-coupon-form .messages-cupons').length>1){
					$('body').find('#discount-coupon-form .messages-cupons:eq(0)').remove();
				}
	...
	$('body').on('click', '.modal-context.rand.cotentmodal', function(event) {
		$('.modal-context').remove();
	});
	$('body').on('click', '.modal-context.rand.cotentmodal .modal-container', function(event) {
		return false;
	});
	$('body').on('click', '.show-hide-form-coupon-link', function(event) {
		$('body').find('.show-hide-form-coupon').addClass('d-none');
		$('body').find('[action="https://www.somproduct.ro/checkout/cart/couponPost"]').addClass('active');
		return false;
	});
	function start_load(){
		$('body').find('.modal-content').addClass('loading');
	}
	function end_load(){
		$('body').find('.modal-content').removeClass('loading');
	}
