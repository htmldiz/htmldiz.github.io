    ...
    function reloaddiscontdata(href){
        ...
        if(discount != '0' && $('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container [data-discont="'+hrefrep+'"]').length < 1){
            if($('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container a[href="'+href+'"]').length > 0){
                // if($('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container [data-discont="'+hrefrep+'"]').length >0){
                    // $('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container [data-discont="'+hrefrep+'"]').remove();
                // }
                    var get_price = $('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container a[href="'+href+'"]').closest('.cart-item').find('.subtotal-col .price:eq(0)').text();
                    get_price = get_price.replace(' lei','');
                    get_price = get_price.replace(',','');
                    get_price = get_price.trim();
                    get_price = Number(get_price);
                    get_price = get_price / 100;
                    discount  = Number(discount);
                    discount_price = get_price / 100 * discount;
                    ...
                    if($('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container [data-discont="'+hrefrep+'"]').length < 1){
                        item_after.after('<div class="discont-item" data-discont="'+hrefrep+'"><span class="discont-t">Today</span> <span class="discont-item-t">-'+discount+'%</span> <span class="discont-save">Save '+discount_price+' lei</span></div>');
                    }else{
                        $('body').find('.modal-context.cotentmodal.cotentmodal-dialog .modal-container [data-discont="'+hrefrep+'"]').html('<span class="discont-t">Today</span> <span class="discont-item-t">-'+discount+'%</span> <span class="discont-save">Save '+discount_price+' lei</span>');
                    }
                // }
            }
        }
                ...
                if($(el).find('.product-name a').length > 0){
                    ...
                    if(discount == '0'){
                        ...
                        .done(function(data) {
                            ...
                            var href = this.url;
                            var hrefrep  = repl_url( href );
                            if($(data).find('.discount span').length){
                                discount = $(data).find('.discount span').text();
    ...
    function reload_modal(){
       ...
       .done(function(done) {
        ...
        if($(done).find('[action="https://www.somproduct.ro/checkout/cart/updatePost"]').length > 1){
            ...
            // console.log(html);
            // console.log('reload_discont done');
            reload_discont(html);
            loading_end();
            // loading_end();
        }
