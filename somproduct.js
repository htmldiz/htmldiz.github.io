...
jQuery(function($) {
        ...
        if($(done).find('[action="https://www.somproduct.ro/checkout/cart/updatePost"]').length > 1){
            ...
            $(done).find('[action="https://www.somproduct.ro/checkout/cart/updatePost"] .cart-items .cart-item a.product-image').each(function(index, el) {
                ...
                if(cheskchref == ''){
                    ...
                    .done(function(data) {
                        var discount = '-1';
                        if($(data).find('.product-essential .discount').length > 0){
                            discount = $(data).find('.product-essential .discount').text();
                        }
                        var hr = this.url.replace('https://www.somproduct.ro/','');
                        hr = hr.trim().replace(new RegExp([' '],'g'),'').toLowerCase().replace(new RegExp('/','g'),'').replace(new RegExp(',','g'),'');
                        setC(hr,discount);
                    });
                }else{
    ...
    $(document).mouseleave(function(event) {
        ...
        tabs                 = Number(tabs);
        var _popupcandisplay = getC('popupcandisplay');
        if(popup_is_display != 'true' && clone != '' && _popupcandisplay == 'true' && tabs < 2 ){
            if($('body').find('.modal-context.rand.cotentmodal.cotentmodal-dialog').length < 1){
                setC('popup_is_display', 'true');
                $('body').append('<div class="modal-context rand cotentmodal cotentmodal-dialog" role="dialog" style="z-index: 999999999999;"><div class="modal-container" data-modal-control="container" role="region" style="width: 570px;"><button class="modal-close" data-modal-control="close"></button><div class="modal-title-container"><div class="modal-title h1" data-modal-control="title" role="heading">Unpaid products in your cart are in high demand.<br />There are only a few unitsremaining in stock.</div></div><div class="modal-content" data-modal-control="content"><div class="form-place"></div><div class="row bottom-cart-container"><div class="info-box">We can\'t guarantee the availability of these products if you leave the site</div><a href="https://www.somproduct.ro/plaseaza-comanda" class="link-complite-cart">Complete your order</a></div><span class="payment-methods">Modalităţi de plată</span></div></div></div>');
           }
