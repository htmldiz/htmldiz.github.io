...
jQuery(function($) {
    $("body").on("click", "#popup-register-form #submit", function(event){
        ...
        if(form.find('[name="custfullname"]').val() == ""){
            can_submit = false;
            form.find('[name="custfullname"]').focus();
            form.find('[name="custfullname"]').before("<p class='fname_valid_text valid_text'>Please fill out this field</p>")
        }
        ...
        if(can_submit == true){
            ...
            $('.fancybox-skin #popup.loading').append('<div class="trial_box"><h3 class="t_title">What you will get in 1$ trial?</h3><ul><li>All property details</li> <li>Owners direct contact information</li> <li>Access to over 1 Million other bargain listings, some as low as $10,000</li></ul></div>');
            setTimeout(function(){
                console.log("true3")
        ...
        var form = $('#popup-register-form');
        $(document).on("focus", '#custfname', function(){
            form.find('.fname_valid_text').remove();
        });
        $(document).on("focus", '#custlname', function(){
